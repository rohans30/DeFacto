...
export default function Home() {
  ...
  const handleInitialize = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/analysis/initialize', {
        method: 'POST',
        body: formData,
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/analysis/continue', {
        method: 'POST',
